...
class PerturbationPredictorNoCellLine(nn.Module):
    """
    Cross-species friendly predictor:
    - remove discrete cell_line token dependency
    - keep control RNA + perturb + dose + ATAC as conditions
    """
    def __init__(
        self,
        n_genes,
        ...
        self.null_atac_token = nn.Parameter(torch.zeros(1, 1, d_model))
        self.cls_token = nn.Parameter(torch.zeros(1, 1, d_model))
        max_seq_len = n_ctrl_tokens + 4  # CLS + ctrl + perturb + dose + atac
        self.pos_embed = nn.Parameter(torch.zeros(1, max_seq_len, d_model))
    ...
    def forward(self, rna_control, perturb, drug_feat=None, dose=None, atac_feat=None):
        if rna_control.dim() == 3 and rna_control.shape[1] == 1:
            rna_control = rna_control.squeeze(1)
        ...
        seq = torch.cat([cls_token, ctrl_tokens, p_token, d_token, atac_token], dim=1)
        seq = seq + self.pos_embed[:, :seq.size(1), :]
        h = self.transformer(seq)
        h = self.final_norm(h)
